...
def download_and_unzip_weights(weights, save_dir):
    if "http" in str(weights):  # weights are in the cloud
        print("Downloading Replicate fine-tune weights")
        st = time.time()
        download_file(str(weights), "training_output.zip")
        print(f"Downloaded weights in {time.time() - st:.3f}")
        print(os.listdir("."))
        print()
    ...
    st = time.time()
    with ZipFile(remote_weights_or_path, "r") as zip_ref:
        zip_ref.extractall(save_dir)
    print(f"Unzipped weights in {time.time() - st:.3f}")
