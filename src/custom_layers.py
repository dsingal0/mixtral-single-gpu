...
from .packing import (
    pack_4bit_u8_common,
    pack_2bit_u8_common,
    unpack_4bit_u8_common,
    unpack_2bit_u8_common,
)
from .triton_kernels import (
    triton_matmul4_transpose,
    triton_matmul3_transpose,
...
class HQQLinearTritonSavable(HQQLinear):
    ...
    def dequantize(self):
        ...
        if (meta["group_size"] is not None) and (meta["nbits"] == 3):
            W_q_p = (
                W_q_p[: meta["group_size"]]
                if (meta["axis"] == 0)
                else W_q_p[:, : meta["group_size"]]
            )
    ...
    def _load_from_state_dict_hook(
        self,
        state_dict,
        prefix,
        local_metadata,
...
class MixtralBLockSparseTop2MLP_HQQ(nn.Module):
    def __init__(
        self, config: MixtralConfig, quant_config: Dict[str, Any], meta1, meta2
    ):
        super().__init__()
